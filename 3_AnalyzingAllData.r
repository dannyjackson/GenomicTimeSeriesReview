...
setwd("/Users/danjack/Library/CloudStorage/OneDrive-UniversityofArizona/McNew\ Lab/collaboration/AmNat_PaperScreening_latest/SearchTermDatafiles")
...
df <- read.csv("final_datatables/alldata.csv")
...
df$Study.system_paperinfo[df$Study.system_paperinfo=="protozoa"] <- "microbes"
...
# create a plot of paper counts by year across categories (Supplemental figure 1)
...
plot1 <- ggplot(df, aes(x = year_paperinfo, y = Experimental.design_paperinfo)) +
  ...
  scale_size_continuous(limits = common_size_limits,  range = c(0, 6)) +
...
pdf(file = "../DraftFigures 2/papercounts_year.pdf",   
    width = 8,
    height = 12) 
